...
router.put('/:id', authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    const category = await prisma.category.update({ where: { id: req.params.id }, data: updateData });

    if (theme) {
      await prisma.categoryTheme.upsert({
        where: { categoryId: req.params.id },
        update: { ...theme },
        create: { categoryId: req.params.id, title: theme.title || category.name, ...theme },
      });
    }
    invalidateCache();
    const full = await prisma.category.findUnique({ where: { id: req.params.id }, include: { theme: true } });
    res.json(full);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
